...
def ica_data(dataset):
    ...
    for i, column in x.items():
        ...
        if i in cycles_to_loop:
            ...
            # plots graph only for ALL cycles
            ...
            if len(ind_peaks[0])==1:
                valpeak2_p1.append(inc_cap_smoothed[n][ind_peaks[0]][0])
                # append NaN when it can't find the second peak
                valpeak2_p2.append(np.nan)

                voltpeak2_1.append(discharge_CC_voltage[n][ind_peaks[0][0]])
                # append NaN when it can't find the second peak
                voltpeak2_2.append(np.nan)
            ...
            if len(ind_peaks[0]) == 6:
                #  get all SIX peaks found
                valpeak2_test1 = inc_cap_smoothed[n][ind_peaks[0]][0]
                valpeak2_test2 = inc_cap_smoothed[n][ind_peaks[0]][1]
                valpeak2_test3 = inc_cap_smoothed[n][ind_peaks[0]][2]
                valpeak2_test4 = inc_cap_smoothed[n][ind_peaks[0]][3]
                valpeak2_test5 = inc_cap_smoothed[n][ind_peaks[0]][4]
                valpeak2_test6 = inc_cap_smoothed[n][ind_peaks[0]][5]




                # put all four peak values into list and select the max and min ones
                list1 = [valpeak2_test1, valpeak2_test2, valpeak2_test3, valpeak2_test4, valpeak2_test5, valpeak2_test6]
                maxval = max(list1)
                minval = min(list1)
