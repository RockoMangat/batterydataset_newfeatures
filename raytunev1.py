...
# train_dataloader = DataLoader(list(zip(X_train_tensor, y_train_tensor)), batch_size=int(config["batch_size"]))
# # train_dataloader = DataLoader(list(zip(X_train_tensor, y_train_tensor)), batch_size=32, shuffle=True)
#
#
# test_dataloader = DataLoader(list(zip(X_test_tensor, y_test_tensor)), batch_size=int(config["batch_size"]))
...
config = []
analysis = tune.run(trainmodel, config={"lr": tune.grid_search([0.001, 0.01, 0.1]), "batch_size": tune.choice([2, 4, 8, 16])})
